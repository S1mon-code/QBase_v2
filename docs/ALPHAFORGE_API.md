...
单策略报告的 Metrics Cards 显示三个 section：

| Section | 说明 |
|---------|------|
| **核心绩效** | Sharpe / 收益 / 回撤 / 波动率 / 持仓统计 |
| **独立交易统计** | 合并 rebalance 调仓，flat→持仓→flat 算一笔。避免 continuous sizing 策略交易次数虚高 |
| **调仓明细统计** | 每笔 round-trip 单独计算，含 partial close / rebalance 粒度 |

**QBase 影响：** `_SignalAdapter` 对 4h/daily 用 `_on_bar_continuous`（每 bar rebalance），对 1h/2h 用 `_on_bar_fixed_entry`。独立交易统计修正了 4h 交易次数虚高于 1h 的问题。

底层：`compute_full_metrics()` 输出两套指标 — 无前缀（independent）和 `rt_` 前缀（round-trip）。
...
| 报告 | 方法 | 适用场景 |
|------|------|---------|
| 单策略 | `reporter.generate()` | 单个策略回测（双视角交易指标） |
