"""MildTrendLongIDailyV45 — Bollinger(25) squeeze release + MACD(16,35,10) bullish.

Economic logic: Bollinger Band squeeze indicates compression — a period of low
volatility that precedes breakout. When bands expand after squeeze and MACD confirms
bullish direction, it signals the start of a new trending move. Signal scales with
MACD strength and bandwidth expansion.
"""
...
class MildTrendLongIDailyV45(TrendingStrategy):
    name = "mild_trend_long_I_daily_v45"
    horizon = "medium"
    direction = "long"
    ...
    def on_init_arrays(self, closes, highs, lows, opens, volumes, oi, datetimes):
        ...
        # Compute bandwidth for squeeze detection
        n = len(self._closes)
    ...
    def _generate_signal(self, bar_index: int) -> float:
        ...
        bb_up = self._bb_upper[bar_index]
        ...
        if any(np.isnan(v) for v in [close, bb_up, macd_l, macd_s, bw, bw_prev]):
            return 0.0

        # Squeeze release: bandwidth expanding
        squeeze_release = bw > bw_prev
        # Bullish breakout: close near or above upper band
    ...
    def get_indicator_panels(self, datetimes):
        return {
            "overlays": [
                self._make_overlay(f"BB Upper", datetimes, self._bb_upper, color="#42a5f5"),
                self._make_overlay(f"BB Mid", datetimes, self._bb_mid, color="#90a4ae",
                                   style="dash"),
                self._make_overlay(f"BB Lower", datetimes, self._bb_lower, color="#42a5f5"),
            ],
            "subplots": [
                self._make_subplot(
                    "MACD",
