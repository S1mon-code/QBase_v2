"""MildTrendShortIDailyV19 — Linear regression slope + Stochastic weak.

Economic logic: Linear regression slope(40) < 0 on daily Iron Ore quantifies
the statistical downtrend, removing noise from the price path. Stochastic(20,5)
< 30 confirms the price is trading near recent lows within that downtrend,
signaling continued bearish pressure.
"""
...
class MildTrendShortIDailyV19(TrendingStrategy):
    """LinReg slope(40) < 0 + Stochastic(20,5) < 30."""

    name = "mild_trend_short_I_daily_v19"
    horizon = "slow"
    direction = "short"
    ...
    def _raw_signal(self, bar_index: int) -> float:
        ...
        signal = -(0.3 + min(0.2, abs(lr) * 0.5))

        # Stochastic weakness confirmation
        if sk < 30:
            signal -= min(0.15, (30 - sk) / 100.0)

        # Extra if both K and D are weak
        if sd < 30:
            signal -= 0.05
