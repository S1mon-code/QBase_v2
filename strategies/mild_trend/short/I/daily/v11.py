"""MildTrendShortIDailyV11 — EMA bearish cross + RSI weak + OBV declining.

Economic logic: EMA(50) crossing below EMA(120) signals a long-term bearish
trend shift on daily Iron Ore. RSI(20) < 45 confirms fading momentum without
being oversold. OBV declining below its SMA validates distribution pressure
...
class MildTrendShortIDailyV11(TrendingStrategy):
    """EMA(50) < EMA(120) + RSI(20) < 45 + OBV < OBV_SMA(50)."""

    name = "mild_trend_short_I_daily_v11"
    horizon = "slow"
    direction = "short"
    ...
    def _raw_signal(self, bar_index: int) -> float:
        ...
        # Base signal from EMA spread
        spread = (es - ef) / es if es != 0 else 0.0
        signal = -(0.25 + min(0.2, spread * 5.0))
