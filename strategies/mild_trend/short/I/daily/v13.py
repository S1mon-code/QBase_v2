"""MildTrendShortIDailyV13 — MACD bearish + Force Index negative.

Economic logic: MACD(20,40,12) line below signal line on daily Iron Ore
identifies bearish momentum in the intermediate-term trend. Force Index(20)
< 0 confirms that volume-weighted price moves are net negative, showing
bears are pushing with conviction.
"""
...
class MildTrendShortIDailyV13(TrendingStrategy):
    """MACD(20,40,12) bearish + Force Index(20) < 0."""

    name = "mild_trend_short_I_daily_v13"
    horizon = "slow"
    direction = "short"
    ...
    def _raw_signal(self, bar_index: int) -> float:
        ...
        # MACD must be bearish (line below signal)
        if ml >= ms:
            return 0.0

        # Base signal from MACD divergence
        macd_gap = abs(ml - ms)
        signal = -(0.3 + min(0.2, macd_gap * 0.5))
