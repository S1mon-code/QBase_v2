"""MildTrendShortIDailyV17 — Vortex bearish + CCI oversold.

Economic logic: Vortex(25) VI- > VI+ on daily Iron Ore identifies bearish
rotational momentum, where downward price spirals dominate upward ones.
CCI(25) < -50 confirms the instrument is trading below its statistical mean
with bearish momentum, validating the short bias.
"""
...
class MildTrendShortIDailyV17(TrendingStrategy):
    """Vortex(25) VI- > VI+ + CCI(25) < -50."""

    name = "mild_trend_short_I_daily_v17"
    horizon = "medium"
    direction = "short"
    signal_dimensions = ["momentum"]
    warmup: int = 50

    vortex_period: int = 25
    cci_period: int = 25
    ...
    def _raw_signal(self, bar_index: int) -> float:
        ...
        # Base signal from vortex spread
        spread = vm - vp
        signal = -(0.3 + min(0.2, spread * 1.5))
