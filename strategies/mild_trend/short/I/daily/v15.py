"""MildTrendShortIDailyV15 — KAMA slope negative + MFI weak.

Economic logic: KAMA(80) with its adaptive smoothing declining signals a
persistent bearish drift in daily Iron Ore. MFI(20) < 40 confirms that
money flow is net negative — buying pressure is insufficient to support
prices, reinforcing the short bias.
"""
...
class MildTrendShortIDailyV15(TrendingStrategy):
    """KAMA(80) slope < 0 + MFI(20) < 40."""

    name = "mild_trend_short_I_daily_v15"
    horizon = "slow"
    direction = "short"
    ...
    def _raw_signal(self, bar_index: int) -> float:
        ...
        # Base from KAMA slope magnitude
        slope_pct = abs(slope) / k_prev if k_prev != 0 else 0.0
        signal = -(0.3 + min(0.2, slope_pct * 300))

        # MFI weakness confirmation
